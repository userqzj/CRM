...
<script type="text/javascript">
	...
	$(function(){
		...
		//页面加载完毕后，取出关联的市场互动信息列表
		showActivityList();

		// 页面加载完毕之后,取出关联的备注信息列表
		showClueList();

		//为关联市场活动模态窗口中的 搜索框绑定事件，通过触发回车键 查询并展现所需市场互动列表
		$("#aname").keydown(function (event) {
			//如果是回车键
			//对于模态窗口 敲回车会默认触发一个事件，强制刷新当前页面并将数据清空
					...
					$.ajax({
						...
						success:function (data) {
							...
							if (data){
								...
								//清楚搜索框中的信息 复选框中的√ 打掉 清空activitySearchBody中的内容
		...
		$.ajax({
			...
			success:function (data){
				...
				$.each(data,function(i,n){
					...
					html += '<font color="gray">线索</font> <font color="gray">-</font> <b>"${c.fullname}${c.appellation}"-"${c.company}"</b> <small style="color: gray;"> "${c.editTime == null?c.createTime:c.editTime}" 由"${c.owner}"</small>';
