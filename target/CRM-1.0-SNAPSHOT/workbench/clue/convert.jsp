	...
	$(function(){
		...
		$("#convertBtn").click(function () {
			/*
				...
				根据为客户创建交易的复选框有没有打√来判断是否需要创建交易
			 */

			if ($("#isCreateTransaction").prop("checked")){

				//alert("需要创建交易")
				//如果需要创建交易，除了要为后台传递clueId之外还要传递交易表单中的信息 金额 预计成交日期 交易名称 阶段 市场活动源(id)
				//window.location.href = "workbench/clue/convert.do?clueId=${param.id}&money=&exceptedDate&name&stage&activityId";
...
<%--		<h4>转换线索 <small><%=fullname%><%=appellation%>-<%=company%></small></h4>--%>
		...
		<%--
			...
			其他所有的隐含对象一概不能省略 如果省略掉了会变成从域对象中取值
		--%>

		<%--${pageContext.request.contextPath}--%>
		<%--
			pageContext 1.当正常页面域使用
						2.变成其他域对象使用  pageContext.setAttribute("str1","aaa",pageContext.REQUEST_SCOPE)
						3.EL表达式中取其他域对象 ${pageContext.request.contextPath}
		--%>
